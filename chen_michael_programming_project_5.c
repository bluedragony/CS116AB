...
void AccumulateForces ()
{
	...
	for (x = 0; x < TOTAL_PARTICLES; x++)	{
		...
		Vec3 v1 = { particles[down].position.x - particles[x].position.x,
		    particles[down].position.y - particles[x].position.y,
		    particles[down].position.z - particles[x].position.z };
		Vec3 v2 = { particles[right].position.x - particles[x].position.x,
		    particles[right].position.y - particles[x].position.y,
		    particles[right].position.z - particles[x].position.z };

		Vec3 normal = Cross(v1, v2);
		Vec3 nn = normalized(normal);
		float d = Dot(nn, wind_Force);
		Vec3 wind = {normal.x*d, normal.y*d, normal.z*d};
		particles[x].acceleration.x += wind.x;
		particles[x].acceleration.y += wind.y;
		particles[x].acceleration.z += wind.z;

		

		Vec3 v3 = { particles[down].position.x - particles[right_down].position.x,
		    particles[down].position.y - particles[right_down].position.y,
		    particles[down].position.z - particles[right_down].position.z };
		Vec3 v4 = { particles[right].position.x - particles[right_down].position.x,
		    particles[right].position.y - particles[right_down].position.y,
		    particles[right].position.z - particles[right_down].position.z };

		Vec3 normal1 = Cross(v3, v4);
		Vec3 nn1 = normalized(normal1);
		float d1 = Dot(nn1, wind_Force);
		Vec3 wind1 = {normal1.x*d1, normal1.y*d1, normal1.z*d1};
		particles[x].acceleration.x += wind1.x;
		particles[x].acceleration.y += wind1.y;
		particles[x].acceleration.z += wind1.z;
	}
...
void collision()	{
	...
	for (x = 0; x < TOTAL_PARTICLES; x++)	{
		Vec3 check = { particles[x].position.x - ball_position.x,
			       particles[x].position.y - ball_position.y,
			       particles[x].position.z - ball_position.z };
...
int dist_travel = 0; // amount of movement for ball.
void display (void)
{
	...
	float move = cos(timer/10)*5;
	ball_position.z = move;
